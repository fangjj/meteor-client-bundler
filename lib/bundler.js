...
const meteorClientModulesDir = Path.resolve(nodeModulesDir, 'meteor-client');
try {
  Fs.statSync(meteorClientModulesDir);
} catch (e) {
  Fs.mkdirSync(meteorClientModulesDir);
}
const orderFileCopy = Path.resolve(meteorClientModulesDir, "order.json");
...
function bundle(options) {
  ...
  if (config.writeRuntimeConfig) {
    ...
    const packFileCopy = Path.resolve(meteorClientModulesDir, "runtime-config.js");
    Fs.writeFileSync(packFileCopy, runtimeconfig);
    order.push("runtime-config.js");
  ...
  program.manifest
    ...
    .forEach(function (pack) {
      ...
      if (config.escapeCommonJS) {
        ...
        const filenameCopy = packFile.substr(packFile.lastIndexOf('/') + 1);
        
        const packFileCopy = Path.resolve(meteorClientModulesDir, filenameCopy);
        Fs.writeFileSync(packFileCopy, packContent);
        order.push(filenameCopy);
  ...
  if (config.disableReload) {
    ...
    const packFileCopy = Path.resolve(meteorClientModulesDir, "post-bundle.js");
    Fs.writeFileSync(packFileCopy, postBundleContent);
    order.push("post-bundle.js");
    
    Fs.appendFileSync(destinationFile, postBundleContent);
  }
  
  Fs.writeFileSync(orderFileCopy, JSON.stringify(order, null, 2));
}
