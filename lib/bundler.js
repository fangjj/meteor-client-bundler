...
var copy = function( src, dst, callback ) {
    var sc = Fs.statSync(src);
    var exists = Fs.existsSync(dst);
    if( !exists && sc.isDirectory()){
        Fs.mkdirSync(dst);
        callback(src, dst);
    } else {
        callback(src, dst);
...
var copyFile = function( src, dst ){
    ...
    if (sc.isDirectory()) {
        ...
        paths.forEach(function( path ){
            ...
            if( st.isFile() ){
                var content = Fs.readFileSync(_src).toString();
                Fs.writeFileSync(_dst, content);
...
function bundle(options) {
  ...
  try {
    var copyDirs = config.copyDirs;
    if (copyDirs && copyDirs.length) {
      copyDirs.forEach(function(itemDir) {
        var srcDir = Path.resolve(cwd, itemDir);
        var tempDist = Path.resolve(tempDir, itemDir);
        copy(srcDir, tempDist, copyFile);
      })
   }
  } catch (err) {
    console.error(err);
